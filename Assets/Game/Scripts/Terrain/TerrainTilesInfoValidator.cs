...
public class TerrainTilesInfoValidator
{
    private TileSetSo[] _tiles;
    private TerrainDataGenerator _terrainData;
    private bool _isErrorFound;
    private Dictionary<float, TileSetSo> _tilesNoiseMin = new();
    private Dictionary<float, TileSetSo> _tilesNoiseMax = new();
    
    public void ValidateTiles(TileSetSo[] tiles, TerrainDataGenerator terrainData)
    {
        _tiles = tiles;
    ...
    private bool IsArrayNullOrEmpty()
    {
        ...
        Debug.LogWarning($"{_terrainData.name} does not contain any tiles");
        return true;
    }

    private bool IsTileEmpty(TileSetSo tile, int index)
    {
        if (tile != null) return false;
        Debug.LogWarning($"{_terrainData.name}: on {index} tile is null");
        return true;
    }
