...
public static class MapHelper
{
    ...
    public static List<Vector2Int> GetChunksIndexesInArea(int chunksCount, Vector2Int center)
    {
        var positions = new List<Vector2Int>();
        var halfArea = chunksCount / 2;
        
        var startX = center.x - halfArea;
        var startY = center.y - halfArea;
    ...
    public static List<Chunk> GetChunksFromIndexes(List<Vector2Int> indexes)
    {
        var chunksWithIndexes = new List<Chunk>();
        var chunks = DS.GetSceneManager<SceneInitializeService>().MapInitializer.TerrainGenerator.GetChunks();
        foreach (var index in indexes)
        {
            if (!chunks.TryGetValue(index, out var chunk)) continue;
            chunksWithIndexes.Add(chunk);
        }
        return chunksWithIndexes;
    }
}